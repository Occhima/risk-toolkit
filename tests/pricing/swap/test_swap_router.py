...
def test_swap_leg_router_routes_known_legs_and_unknown_to_fallback() -> None:
    market = MarketSnapshot(
        ...
        fixings=pl.DataFrame(
            {"id_indexador": [3], "fixing_date": [date(2026, 6, 3)], "fixing_value": [100.0]}
        ).lazy(),
    ...
    legs = pl.DataFrame(
        {
            ...
            "pay_receive": ["RECEIVE", "RECEIVE", "RECEIVE", "RECEIVE"],
    ...
    out = cast(
        pl.DataFrame,
        swap_leg_router.compute_for(legs, market=market, output_profile="pricing").collect(),
    )
