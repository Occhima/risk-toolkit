...
def year_fraction_term(g: Formula, *, payment_days: Term[int]) -> Term[float]:
    @g.formula(
        tags=("time",),
        symbol="T",
        latex=r"\frac{d}{252}",
...
def discount_factor_term(
    g: Formula, *, zero_rate: Term[float], year_fraction: Term[float]
) -> Term[float]:
    @g.formula(
        tags=("discounting",),
        symbol="DF",
...
def present_value_term(
    g: Formula, *, future_value: Term[float], discount_factor: Term[float], name: str = "pv"
) -> Term[float]:
    @g.formula(
        name=name,
        tags=("pricing",),
