...
These functions only *render* a :class:`~schenberg.core.graph.FormulaGraph` — its
LaTeX (derived from the symbolic IR, never hand-written) and its Mermaid
dependency diagram — into the Markdown that Quarto turns into MathJax and Mermaid.
...
def show_formulas(graph: FormulaGraph, *, title: str = "Formulas") -> None:
...
def show_formula(graph: FormulaGraph, term: str) -> None:
    """Emit a single term's LaTeX as a display equation."""
    print(f"$$ {graph.formula_of(term)} $$\n")


def show_graph(graph: FormulaGraph) -> None:
...
def show_explain(graph: FormulaGraph, *, view: str) -> None:
