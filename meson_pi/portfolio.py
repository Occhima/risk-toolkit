...
from .swap.schemas import SwapInput
from .swap.pricing_graph import price_swap
...
pnl_attribution_graph.with_outputs("attribution",
                                   carry_pnl="carry_pnl", market_pnl="market_pnl",
                                   total_pnl="total_pnl")
...
# ---------------------------------------------------------------------------
@pa.check_types(lazy=True)
def compute_pnl(positions: LazyFrame[Positions],
                market_today: MarketSnapshot, market_prev: MarketSnapshot, *,
                value: ValueFn) -> LazyFrame[PnLReport]:
    ...
    return legs.group_by("book").agg(pnl=pl.col("pnl").sum())


@pa.check_types(lazy=True)
def compute_dv01(positions: LazyFrame[Positions],
                 market: MarketSnapshot, bumped: MarketSnapshot, *,
                 value: ValueFn) -> LazyFrame[RiskReport]:
    ...
    return legs.group_by("book").agg(dv01=pl.col("dv01").sum())
